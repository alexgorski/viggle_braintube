...
#Take the teaser title and append it with the show title
  def change_title
    self.title = self.title + Show.find(:id => self.show_id).title
    self.save

  end

#Take the teaser body and if there are character names, randomly substitute character names
# for the generic names
  def change_body(num_of_characters)
    ...
    for i in 0..num_of_characters
      x = chars[rand(char.size)]
      question_characters.include? x ? nil : question_character << x
    end
    
    for i in 0..num_of_characters
      bdy.gsub!("Person#{i}",question_characters[i])
    end
    self.save
  end
